...
*       text
*----------------------------------------------------------------------*
*      -->P_GT_BOOKINGS_MOD  text
*----------------------------------------------------------------------*
FORM modify_bookings  USING pt_bookings TYPE gty_t_sbook
                            ps_conn TYPE sdyn_conn.

  CALL FUNCTION 'UPDATE_SBOOK'
    EXPORTING
      it_sbook             = pt_bookings
   EXCEPTIONS
     UPDATE_FAILURE       = 1
     OTHERS               = 2.
  ...
  CASE sy-subrc.
      ...
      CALL FUNCTION 'UPDATE_SFLIGHT'
        EXPORTING
          iv_carrid              = ps_conn-carrid
          iv_connid              = ps_conn-connid
          iv_fldate              = ps_conn-fldate
       EXCEPTIONS
         UPDATE_FAILURE         = 1
         FLIGHT_FULL            = 2
         FLIGHT_NOT_FOUND       = 3
        ...
        CASE sy-subrc.
          "여기서 E는 모두 Auto commit.
          WHEN 0.
            MESSAGE s034 WITH ps_conn-carrid
                              ps_conn-connid
                              ps_conn-fldate.
          WHEN 1.
            MESSAGE e044 WITH ps_conn-carrid
                               ps_conn-connid
                               ps_conn-fldate.
          WHEN 2.
            MESSAGE e045.
        ...
        WHEN 1 OR 2.
          MESSAGE a044 WITH ps_conn-carrid
                            ps_conn-connid
                            ps_conn-fldate.
  ENDCASE.
